...
must_haves:
  truths:
    - "All iframe communication works reliably without dropped messages or duplicate send paths"
...
- **`handleEditHover(e: MouseEvent)`**: If `mode !== "edit"` or no overlay, return. Find deepest `[data-block-id]` from `e.target`. If a section is found: get its `getBoundingClientRect()`, set `hoverOverlay.style.display = "block"`, position overlay via `top/left/width/height` from the rect, and send `{ type: "deco:block-hover", blockId, rect }` to parent. If NO section is found: set `hoverOverlay.style.display = "none"` and send `{ type: "deco:block-hover", blockId: null, rect: null }` to parent.
