...
<objective>
Wire click-to-select from the iframe overlay to the sidebar prop editor, add the edit/interact mode toggle, implement iframe disconnect detection with reconnect overlay, and verify live prop editing works end-to-end through the consolidated bridge.

Purpose: Make the visual editor interactive -- users can click sections to edit them, toggle between editing and browsing, and recover from dev server crashes.

Output: Fully interactive preview panel with mode toggle, click-to-select, deselect-on-click-away, disconnect overlay, and live prop editing.
</objective>
...
Pass `mode` to `useIframeBridge` options. Pass `mode` and `onModeChange={handleModeChange}` to PreviewPanel.
  </action>
  <verify>
...
8. The full flow works: iframe sends deco:block-clicked -> bridge -> setSelectedBlockId -> prop editor opens
</verification>
...
- Live prop editing: prop changes in the editor send deco:update-block through the bridge to the iframe
</success_criteria>
