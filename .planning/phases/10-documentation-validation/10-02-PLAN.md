...
Full end-to-end validation of the deco CMS site editor using anjo.chat as the reference implementation. All 10 validation items have been tested and any bugs discovered have been fixed.
  ...
  <action>
Run through the manual verification steps to confirm the complete end-to-end flow:
...
3. Navigate to the site-editor plugin — confirm anjo.chat is connected
4. Go to Sections page — confirm blocks are listed (Hero, Features, etc.)
5. Go to Loaders page — confirm empty state renders cleanly
6. Open a page in the composer — confirm iframe preview shows anjo.chat
7. Click a section in the preview — confirm it gets selected and props appear in sidebar
8. Edit a text prop — confirm the preview updates live
9. Save — confirm the change persists in `.deco/pages/page_home.json`
  </action>
  <verify>
- All 9 manual verification steps complete without errors
- Connection setup works smoothly from plugin empty state
- Sections list displays scanned blocks correctly
- Loaders page shows appropriate empty state
- Live preview renders anjo.chat home page
- Click-to-select interaction functions properly
- Prop editing reflects in preview within 1 second
- Changes persist to disk after save
  </verify>
  <done>
Human has confirmed all validation items work as expected. The end-to-end demo using anjo.chat successfully demonstrates connection setup, block scanning, sections listing, empty loaders state, live preview, click-to-select, prop editing, and save-to-git functionality.
  </done>
  <how-to-verify>
1. Open Mesh admin at http://localhost:4000 (start with `bun run dev` in mesh repo if not running)
2. Start anjo.chat dev server (`bun run dev` in anjo.chat repo)
