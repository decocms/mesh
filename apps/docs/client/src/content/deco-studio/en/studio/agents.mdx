...
**Agents** are single-purpose virtual MCPs optimized for specific tasks. While [Projects](/en/studio/projects) provide general-purpose work environments, agents focus narrowly on accomplishing one thing well.
...
- **Tools** - Actions the agent can take (select from [connections](/en/studio/connections) or define inline)
- **Resources** - Additional context and documentation
- **Prompts** - Workflow templates

[decopilot](/en/studio/decopilot/overview) uses the **description** field to understand when to spawn each agent—just like it uses skill descriptions to know when to invoke skills. This makes agents discoverable and automatically available when relevant to the task at hand.

Since agents are [Virtual MCPs](/en/studio/virtual-mcps), you can compose them from existing connections, define capabilities inline, or mix both approaches.
...
deco Studio includes built-in agents for common mesh operations. These agents use the **Management MCP**—a self-referential MCP that exposes mesh management capabilities through the MCP protocol.
...
**Next steps**: Learn about [Virtual MCPs](/en/studio/virtual-mcps) to understand the foundation, explore [decopilot](/en/studio/decopilot/overview) to see how agents are spawned dynamically, or set up [Automations](/en/studio/automations) for unattended agent execution on schedules and events.
