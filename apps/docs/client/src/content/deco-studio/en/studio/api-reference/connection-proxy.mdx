...
description: Connect MCP clients directly to upstream MCP servers through deco Studio
icon: Network
---
...
The **Connection Proxy** endpoint (`/mcp/:connectionId`) enables MCP clients (like Cursor, Claude Desktop, or custom applications) to connect directly to a specific upstream MCP server through deco Studio. This endpoint acts as a secure, observable proxy that handles authentication, authorization, credential management, and monitoring.
...
1. **Authentication**: deco Studio verifies the client's credentials (OAuth token or API key)
2. **Connection Lookup**: deco Studio retrieves the connection configuration and verifies it's active
3. **Authorization**: deco Studio checks if the client has permission to invoke the requested tool
4. **Credential Injection**: deco Studio decrypts stored credentials and injects them into the upstream request
5. **Request Proxying**: deco Studio forwards the MCP request to the upstream server
6. **Monitoring**: deco Studio logs metrics (duration, status, errors) for observability
7. **Response Return**: deco Studio returns the upstream server's response to the client
...
The Connection Proxy enforces fine-grained authorization using deco Studio's permission system:
...
| `/mcp/self` | Management operations | deco Studio management tools only |
...
- [**Connections**](/en/studio/connections) - Understanding the Connection abstraction
- [**Virtual MCPs**](/en/studio/virtual-mcps) - Compose multiple connections into agents
- [**API Keys**](/en/studio/api-keys) - Authentication setup
- [**Monitoring**](/en/studio/monitoring) - Observability and metrics
