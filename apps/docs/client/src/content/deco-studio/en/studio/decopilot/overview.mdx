---
title: Overview
description: The agentic AI assistant that runs on deco Studio, operating through distributed MCP architecture
icon: Sparkles
---
...
The power comes from **[virtual MCPs](/en/studio/virtual-mcps)**—decopilot can create and modify its own capabilities dynamically as it works, adapting to your needs in real-time. This programmable foundation makes decopilot not just a tool user, but a tool builder.

<Callout type="tip" title="New to decopilot?">
Start with the [Quick Start](/en/studio/decopilot/quickstart) guide to see decopilot in action with practical workflows.
</Callout>
...
You can always check your current context usage in the decopilot UI to understand what's consuming space and when to spawn an agent for complex work. See [Context](/en/studio/decopilot/context) for token budgets, optimization strategies, and practical examples.

## Tasks

As you work, decopilot can delegate specialized work to **[agents](/en/studio/agents)**—focused assistants optimized for specific tasks like customer support, research, or data analysis. When you spawn an agent, it gets its own fresh context to work independently, then reports back with a summary.
...
Learn more about when to spawn agents and spawning patterns in [Tasks](/en/studio/decopilot/tasks-and-spawning).
...
Your current scope is determined by where you are in the deco Studio UI, and decopilot automatically loads the appropriate capabilities for that context. The three scope levels are:

- **Org scope**: The default level with organization-wide [connections](/en/studio/connections), resources, and [agents](/en/studio/agents). Always available as the foundation.
- **Project scope**: Adds project-specific tools, resources, and prompts from the [project's virtual MCP](/en/studio/virtual-mcps). Inherits org scope.
- **Agent scope**: Adds specialized instructions and bounded tools when working with a specific [agent](/en/studio/agents). Inherits org or project scope.
...
- **`read_resource`**: Read project documentation and resources
- **`read_prompt`**: Read reusable prompt templates
- **`read_tool_output`**: Filter large tool outputs using regex patterns
- **`propose_plan`**: Propose execution plans for user approval

These built-in tools are always available and provide the foundation for working with virtual MCPs. Scope tools from your connected MCP servers (Shopify, inventory systems, customer service platforms, etc.) are available based on the connections in the current scope's Virtual MCP.

See [Tools](/en/studio/decopilot/tools) for complete specifications, usage patterns, and examples of coordinating tools across multiple services.

## Next Steps

Ready to learn more? Explore these guides to understand how decopilot works:

**[Quick Start](/en/studio/decopilot/quickstart)**: See decopilot in action with step-by-step workflows. Start here for hands-on examples.

**[Context](/en/studio/decopilot/context)**: Learn strategies for managing token usage, when to spawn agents, and how to optimize context.

**[Tasks](/en/studio/decopilot/tasks-and-spawning)**: Understand task lifecycles, agent selection, and patterns for parallel execution.

**[Tools](/en/studio/decopilot/tools)**: Complete reference for built-in tools and working with target virtual MCP tools.

**[Scopes](/en/studio/decopilot/scopes)**: Deep-dive into scope-based routing and how org/project/agent contexts work.

---

Decopilot brings proven agentic patterns to MCP orchestration. By coordinating work across distributed MCP servers while operating on editable virtual MCPs, decopilot enables complex workflows that span multiple services—all through standardized MCP protocol. This is the agentic assistant that runs on deco Studio.
