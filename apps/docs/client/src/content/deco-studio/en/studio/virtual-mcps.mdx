...
In deco Studio, a **Virtual MCP** is a reusable package of capabilities—[tools, resources, and prompts](/en/studio/concepts#mcp-protocol-primitives)—remixed from multiple [connections](/en/studio/connections).
...
Virtual MCPs are the **backbone of deco Studio**—powering [Projects](/en/studio/projects), [Agents](/en/studio/agents), and organizational boundaries for how capabilities are packaged and composed.
...
**Remixing** lets you select and combine tools, resources, and prompts from multiple [connections](/en/studio/connections).

A **connection** is how you wire up an external MCP server to deco Studio (see [Connections](/en/studio/connections) for details). Once you have connections set up, remixing works like this:
...
- **Feature gating and validation** that wrap connection tools with authorization logic (see [Authorization and Roles](/en/studio/user-management))
...
These patterns enable **authorization enforcement**, **input validation**, **audit logging** (see [Monitoring](/en/studio/monitoring)), and **workflow orchestration** across your MCP infrastructure.
...
When aggregating capabilities from multiple connections, deco Studio uses **connection-based namespacing** to avoid collisions. Each connection gets a unique ID, and all capabilities from that connection are prefixed with this ID.
...
**Automatic content rewriting**: When resources are read through a virtual MCP, deco Studio automatically rewrites internal resource links to use the namespaced format. This ensures resources can reference each other correctly.
...
Virtual MCPs provide the **foundational capability** that powers specialized features across deco Studio. All of these inherit full [monitoring capabilities](/en/studio/monitoring) for audit trails and observability.
...
- **[Agents](/en/studio/agents)**: Single-purpose capability packages optimized for specific tasks
- **[Projects](/en/studio/projects)**: Team-scoped capability packages for organizational boundaries
...
- **Foundation for specialization**: Building block for [Projects](/en/studio/projects) and [Agents](/en/studio/agents)
...
<Callout type="info">
Start with [Connections](/en/studio/connections) to understand how to wire up external MCP servers first.
</Callout>

**Ready to build?** Follow the [Quickstart](/en/studio/quickstart) for step-by-step setup.

**Building for teams?** Learn about [Projects](/en/studio/projects) for team-scoped virtual MCPs, or [Agents](/en/studio/agents) for single-purpose capability packages.

**Need authorization controls?** See [Authorization and Roles](/en/studio/user-management) for securing virtual MCP access with feature gating and policies.
