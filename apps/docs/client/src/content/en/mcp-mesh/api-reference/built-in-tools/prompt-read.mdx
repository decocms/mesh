...
- Set both `lineStart` and `lineEnd` to 0 to read entire prompt
...
prompt_read("code-review-workflow", 0, 0)
```
...
prompt_read("deployment-steps", 5, 15)
```
...
prompt_read("feature-workflow", 0, 0)

// Read bug triage workflow
prompt_read("bug-triage", 0, 0)

// Read code review checklist
prompt_read("review-checklist", 0, 0)
```
...
const workflow = prompt_read("deploy-to-production", 0, 0)
...
const template = prompt_read("bug-fix-template", 0, 0)
...
const securityPrompt = prompt_read("security-review", 0, 0)
