---
title: resource_read
description: Read a resource from the current scope
icon: FileText
---

## Overview

Reads a resource from the current scope. Supports scope inheritance (project inherits from org, agent inherits from project+org). Resources can be documentation, guidelines, code snippets, or any contextual information.

## Availability

- **Tasks**: ✓ Available
- **Subtasks**: ✓ Available

## Signature

```typescript
resource_read(
  resource_uri: string,
  lineStart?: number,
  lineEnd?: number
)
```

## Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `resource_uri` | `string` | Required | URI identifier for the resource |
| `lineStart` | `number` | Optional | Starting line number (default: 0) |
| `lineEnd` | `number` | Optional | Ending line number (default: 20) |

## Returns

Resource content and metadata:
- **content**: The resource text content
- **uri**: Resource URI
- **metadata**: Additional resource information

## Behavior

- Supports scope inheritance:
  - Project resources inherit from organization resources
  - Agent resources inherit from project + organization resources
- Set both `lineStart` and `lineEnd` to 0 to read entire resource
- Default behavior reads first 20 lines (lines 0-20)
- Line numbers are zero-indexed

## Example Usage

### Read Entire Resource

```typescript
// Read entire coding guidelines document
resource_read("coding-guidelines.md", 0, 0)
```

### Read Specific Lines

```typescript
// Read lines 10-30 of API documentation
resource_read("api-docs.md", 10, 30)
```

### Read Beginning of Resource

```typescript
// Read first 20 lines (default)
resource_read("onboarding-guide.md")
```

### Resource Types

```typescript
// Read coding standards
resource_read("standards/typescript-style.md", 0, 0)

// Read deployment procedures
resource_read("procedures/deployment-checklist.md", 0, 0)

// Read API documentation
resource_read("docs/rest-api-reference.md", 0, 0)
```

## Use Cases

### Context-Driven Execution

Read guidelines before executing work:

```typescript
// Read coding standards
const standards = resource_read("coding-standards.md", 0, 0)

// Apply standards when generating code
// ... code generation logic
```

### Documentation Reference

Access reference documentation during task:

```typescript
// Read API docs
const apiDocs = resource_read("api-reference.md", 0, 0)

// Use docs to make correct API calls
// ... API call logic
```

### Procedure Compliance

Follow documented procedures:

```typescript
// Read deployment checklist
const checklist = resource_read("deployment-checklist.md", 0, 0)

// Follow each step in the checklist
// ... deployment logic
```

## Scope Inheritance

Resources follow scope hierarchy:

1. **Agent scope**: Inherits from project + organization
2. **Project scope**: Inherits from organization
3. **Organization scope**: Base level resources

When reading a resource, the system searches from most specific (agent) to least specific (organization) scope.

## Related

- [**prompt_read**](/en/mcp-mesh/api-reference/built-in-tools/prompt-read) - Read prompt templates
- [Context Documentation](/en/mcp-mesh/decopilot/context)
- [Built-in Tools Overview](/en/mcp-mesh/api-reference/built-in-tools)
