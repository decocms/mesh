...
An **Assistant** is a **virtual MCP server** that aggregates multiple connections into a single MCP surface:
...
You can also omit `:gatewayId` and rely on the organization's default Assistant by providing one of these headers:
...
Assistants also control **how tools are exposed** to clients (useful when tool surfaces get large).
...
## The default Assistant (auto-created per org)

Every new organization gets a **Default Assistant**:
...
- **Default behavior**: everything else in the org is included — so as you connect Integrations, this endpoint becomes the "all tools in the org" Assistant
