...
Agents are virtual MCPs stored as connections with `connection_type = 'VIRTUAL'`. They use the same endpoint pattern as regular connections. Supports `?mode=` query parameter for tool selection strategy (passthrough, smart_tool_selection, code_execution).
