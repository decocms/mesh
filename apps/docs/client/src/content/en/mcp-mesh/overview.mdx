...
**decocms** is a **control plane for MCP traffic**. It sits between MCP clients (Cursor, Claude Desktop, custom agents) and MCP servers, providing a centralized layer for **authentication, authorization, credential management, and observability**.
...
decocms centralizes these concerns once, so you can operate MCP traffic like any other production surface.
...
## What decocms centralizes

- **Routing + execution**: which MCP to call and how to authenticate it
- **Policy enforcement**: who can access which tools (and through which virtual MCPs)
- **Observability**: logs, latency, errors, and request context across tool calls
- **Runtime strategies (virtual MCPs)**: different exposure strategies as tool surfaces grow
