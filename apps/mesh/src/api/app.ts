...
async function autoStartConnectionsByTitle(
  ...
  for (const conn of connections) {
    try {
      ...
      // But re-throw spawn failures (command not found, script not found, etc.)
      try {
        await proxy.client.listPrompts();
      } catch (promptError) {
        const errorMsg = String(promptError);
        // Re-throw if this is a spawn failure, not just "prompts not supported"
        if (
          errorMsg.includes("Spawn failed") ||
          errorMsg.includes("spawn_failed")
        ) {
          throw promptError;
        }
        // Ignore other errors (Method not found, etc.) - spawn succeeded
      }

      console.log(`[AutoStart] ✓ ${conn.title} started`);
    } catch (error) {
      // Clean error log - just the message, no stack trace
      const msg = error instanceof Error ? error.message : String(error);
      console.warn(`[AutoStart] ✗ ${conn.title} failed: ${msg}`);
    }
  }
}
