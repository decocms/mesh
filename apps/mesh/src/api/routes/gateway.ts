 ...
 * - If gatewayId is omitted: use default Assistant for org (from x-org-id or x-org-slug header)
 */
app.all("/gateway/:gatewayId?", async (c) => {
  ...
  try {
    ...
    if (gatewayId) {
      ...
      // Load default Assistant for org from headers
      ...
      if (orgId) {
        ...
        return c.json(
          {
            error:
              "Gateway ID required, or provide x-org-id or x-org-slug header for default Assistant",
          },
          400,
        );
      }
    ...
    if (!gateway) {
      ...
      return c.json(
        { error: "No default Assistant configured for this organization" },
        404,
      );
    }
