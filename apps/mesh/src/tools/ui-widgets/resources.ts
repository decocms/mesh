...
<body>
  ...
  "ui://mesh/badge": {
    name: "Badge",
    description: "Badge with multiple variants for status and labels",
    exampleInput: {
      badges: [
...
<head>
  <style>
    ${baseCSS}
    .container { height: 100%; display: flex; align-items: center; justify-content: center; flex-wrap: wrap; padding: 16px; gap: 8px; }
    .badge { display: inline-flex; align-items: center; padding: 2px 10px; border-radius: 9999px; font-size: 12px; font-weight: 500; transition: all 0.15s; }
    .badge.default { background: ${tokens.primary}; color: white; }
    .badge.secondary { background: ${tokens.bgSubtle}; color: ${tokens.text}; }
    .badge.destructive { background: ${tokens.destructive}; color: white; }
    .badge.outline { background: transparent; border: 1px solid ${tokens.border}; color: ${tokens.text}; }
    .badge.success { background: ${tokens.success}; color: white; }
    .badge.warning { background: ${tokens.warning}; color: white; }
  ...
  <script>
    window.addEventListener('message', e => {
      ...
      if (msg.method === 'ui/initialize') {
        ...
        if (input.badges) {
          container.innerHTML = input.badges.map(b => 
            '<span class="badge ' + (b.variant || 'default') + '">' + b.text + '</span>'
          ).join('');
        } else if (input.text) {
          container.innerHTML = '<span class="badge ' + (input.variant || 'default') + '">' + input.text + '</span>';
        }
        parent.postMessage(JSON.stringify({ jsonrpc: '2.0', id: msg.id, result: {} }), '*');
      }
    });
...
<head>
  <style>
    ...
    .container { height: 100%; padding: 16px; display: flex; flex-direction: column; gap: 12px; }
    .skeleton { background: linear-gradient(90deg, ${tokens.bgSubtle} 25%, ${tokens.border} 50%, ${tokens.bgSubtle} 75%); background-size: 200% 100%; animation: shimmer 1.5s infinite; border-radius: 6px; }
    .card .header { display: flex; align-items: center; gap: 12px; margin-bottom: 12px; }
    .card .avatar { width: 40px; height: 40px; border-radius: 9999px; flex-shrink: 0; }
    .card .title { height: 16px; flex: 1; }
    .card .line { height: 12px; margin-bottom: 8px; }
    .card .line:last-child { width: 60%; }
    .list .item { display: flex; align-items: center; gap: 12px; padding: 12px 0; border-bottom: 1px solid ${tokens.border}; }
    .list .item:last-child { border-bottom: none; }
    .list .circle { width: 32px; height: 32px; border-radius: 9999px; flex-shrink: 0; }
    .list .text { flex: 1; height: 14px; }
    .text-lines .line { height: 14px; margin-bottom: 10px; }
    .text-lines .line:nth-child(3n) { width: 80%; }
    .text-lines .line:nth-child(2n) { width: 90%; }
  </style>
</head>
<body>
  <div class="container" id="container"></div>
  <script>
    function renderCard(lines) {
      return '<div class="card"><div class="header"><div class="skeleton avatar"></div><div class="skeleton title"></div></div>' +
        Array(lines || 3).fill('<div class="skeleton line"></div>').join('') + '</div>';
    }
    function renderList(items) {
      return '<div class="list">' + Array(items || 3).fill('<div class="item"><div class="skeleton circle"></div><div class="skeleton text"></div></div>').join('') + '</div>';
    }
    function renderText(lines) {
      return '<div class="text-lines">' + Array(lines || 4).fill('<div class="skeleton line"></div>').join('') + '</div>';
    }
    window.addEventListener('message', e => {
      let msg; try { msg = JSON.parse(e.data); } catch { return; }
      if (msg.method === 'ui/initialize') {
        ...
        if (variant === 'card') container.innerHTML = renderCard(input.lines);
        else if (variant === 'list') container.innerHTML = renderList(input.items);
        else container.innerHTML = renderText(input.lines);
        parent.postMessage(JSON.stringify({ jsonrpc: '2.0', id: msg.id, result: {} }), '*');
      }
    });
  </script>
