...
export const COLLECTION_VIRTUAL_MCP_LIST = defineTool({
  ...
  description: "List all Virtual MCPs in the organization",
  ...
  handler: async (input, ctx) => {
    ...
    if (input.where) {
      filtered = filtered.filter((vmcp) =>
        evaluateWhereExpression(vmcp, input.where!),
      );
    }
...
/** @deprecated Use COLLECTION_VIRTUAL_MCP_LIST instead */
export const COLLECTION_GATEWAY_LIST = COLLECTION_VIRTUAL_MCP_LIST;
