...
/**
 * Returns the execution metadata for the workflow execution being tracked.
 */
export function useExecutionMetadata() {
  const trackingExecutionId = useTrackingExecutionId();
  const { item: executionItem, isLoading } =
    usePollingWorkflowExecution(trackingExecutionId);

  if (!trackingExecutionId || !executionItem) {
    return {
      isTracking: !!trackingExecutionId,
      isLoading,
      status: undefined,
      startedAt: undefined,
  ...
  const startedAt = executionItem.start_at_epoch_ms
    ? new Date(executionItem.start_at_epoch_ms)
    : undefined;
  const completedAt = executionItem.completed_at_epoch_ms
    ? new Date(executionItem.completed_at_epoch_ms)
    : undefined;
  const durationMs =
