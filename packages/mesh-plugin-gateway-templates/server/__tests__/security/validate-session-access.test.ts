...
function createMockStorage(
  session: GatewayTemplateSessionEntity | null,
...
function createTestSession(
  overrides?: Partial<GatewayTemplateSessionEntity>,
...
describe("validateSession", () => {
  it("allows access to valid session", async () => {
    ...
    const result = await validateSession("gtsess_test", storage);
    expect(result.id).toBe("gtsess_test");
  });
  ...
  it("rejects access to non-existent session", async () => {
    ...
    await expect(
      validateSession("gtsess_nonexistent", storage),
    ).rejects.toThrow(SessionAccessError);

    try {
      await validateSession("gtsess_nonexistent", storage);
    } catch (err) {
  ...
  it("rejects access to expired session", async () => {
    ...
    await expect(validateSession("gtsess_test", storage)).rejects.toThrow(
      SessionAccessError,
    );

    try {
      await validateSession("gtsess_test", storage);
    } catch (err) {
...
describe("validateSessionForConfiguration", () => {
  it("allows configuration of pending session", async () => {
    ...
    const result = await validateSessionForConfiguration(
      "gtsess_test",
      storage,
    );
  ...
  it("rejects configuration of completed session", async () => {
    ...
    await expect(
      validateSessionForConfiguration("gtsess_test", storage),
    ).rejects.toThrow(SessionAccessError);

    try {
      await validateSessionForConfiguration("gtsess_test", storage);
    } catch (err) {
...
describe("validateConnectionBelongsToSession", () => {
  it("allows access to connection with matching session ID", () => {
    ...
    const connection = {
      metadata: {
        [METADATA_KEYS.SESSION_ID]: "gtsess_test",
      },
    };
  ...
  it("allows access to connection with matching external user ID and template ID", () => {
    ...
    const connection = {
      metadata: {
        ...
        [METADATA_KEYS.TEMPLATE_ID]: "gtpl_test",
      },
    };
  ...
  it("rejects connection from different session", () => {
    ...
    const connection = {
      metadata: {
        [METADATA_KEYS.SESSION_ID]: "gtsess_other",
        [METADATA_KEYS.EXTERNAL_USER_ID]: "other_user",
        [METADATA_KEYS.TEMPLATE_ID]: "gtpl_other",
      },
    };
