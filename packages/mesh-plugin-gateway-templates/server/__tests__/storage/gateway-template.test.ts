/**
 * Gateway Template Storage Tests
 */
...
import { migration } from "../../migrations/001-gateway-templates";
import type { GatewayTemplatesDatabase } from "../../storage/types";

// Test database setup
let db: Kysely<GatewayTemplatesDatabase>;
let storage: GatewayTemplateStorage;
...
beforeEach(async () => {
  ...
  db = new Kysely<GatewayTemplatesDatabase>({
  ...
  storage = new GatewayTemplateStorage(db);
});
...
describe("GatewayTemplateStorage", () => {
  describe("create", () => {
    it("creates a template with required_apps", async () => {
      ...
      expect(template.id).toStartWith("gtpl_");
  ...
  describe("findById", () => {
    it("returns null for non-existent template", async () => {
      const template = await storage.findById("gtpl_nonexistent");
