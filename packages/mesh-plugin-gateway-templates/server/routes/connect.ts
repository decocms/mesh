/**
 * Gateway Templates Plugin - Connect API Routes
 *
 ...
 *
 * Routes:
 * - GET  /api/gateway-templates/sessions/:sessionId - Get session status and apps
 * - POST /api/gateway-templates/sessions/:sessionId/provision - Create connection for an app
 * - POST /api/gateway-templates/sessions/:sessionId/configure - Mark app as configured
 * - POST /api/gateway-templates/sessions/:sessionId/complete - Complete the session
 */

import { Hono } from "hono";
import type { ServerPluginContext } from "@decocms/bindings/server-plugin";
import type { Kysely } from "kysely";
import { GatewayTemplateStorage } from "../storage/gateway-template";
import { GatewayTemplateSessionStorage } from "../storage/gateway-template-session";
import type {
...
async function createConnectionFromApp(
  ...
  const connectionData = {
    ...
    metadata: JSON.stringify({
      gateway_template_session_id: sessionId,
...
function errorResponse(
  ...
  console.error("[GatewayTemplates] Connect API error:", error);
  return c.json(
...
export function connectRoutes(app: Hono, ctx: ServerPluginContext): void {
  ...
  const templates = new GatewayTemplateStorage(db);
  const sessions = new GatewayTemplateSessionStorage(db);
  const storage = { templates, sessions };

  // ============================================================================
  // GET /api/gateway-templates/sessions/:sessionId - Get session and app statuses
  // ============================================================================
  app.get("/api/gateway-templates/sessions/:sessionId", async (c) => {
    try {
      ...
      try {
        ...
        if (!template.created_by) {
          return c.json(
            {
              error:
                "Template is missing created_by - cannot create connections",
            },
            500,
          );
        }
  ...
  app.post(
    ...
    async (c) => {
      try {
        ...
        const existingStatus = session.app_statuses[body.app_name];
        const connectionId =
          body.connection_id ?? existingStatus?.connection_id;
  ...
  // ============================================================================
  // POST /api/gateway-templates/sessions/:sessionId/complete - Finalize setup
  // ============================================================================
  app.post("/api/gateway-templates/sessions/:sessionId/complete", async (c) => {
    try {
      const sessionId = c.req.param("sessionId");
