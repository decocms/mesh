/**
 * Gateway Templates Plugin - Completion Service
 *
...
/**
 * Complete a gateway template session.
 *
...
export async function completeSession(
  ...
  template: GatewayTemplateEntity,
  ...
  try {
    ...
    if (ctx.eventBus) {
      try {
        ...
        console.error(
          "[GatewayTemplates] Failed to emit completion event:",
          err,
        );
      }
    }
      ...
      try {
        const webhookResponse = await fetch(template.webhook_url, {
          ...
          headers: {
            ...
            "X-Gateway-Template-Event": template.event_type,
          },
        ...
        if (!webhookResponse.ok) {
          console.error(
            "[GatewayTemplates] Webhook returned error:",
        ...
        console.error("[GatewayTemplates] Failed to call webhook:", err);
      }
    }
    ...
    console.error("[GatewayTemplates] Completion failed:", err);
    throw err;
  }
}
