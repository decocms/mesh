/**
 * Gateway Template Storage Implementation
 *
 * Handles CRUD operations for gateway templates using Kysely.
 */

import type { Kysely, Insertable, Updateable } from "kysely";
import { generatePrefixedId } from "./utils";
import type {
  GatewayTemplateEntity,
  GatewayTemplateCreateInput,
  GatewayTemplateUpdateInput,
...
export class GatewayTemplateStorage {
   ...
   */
  async create(
    data: GatewayTemplateCreateInput,
  ): Promise<GatewayTemplateEntity> {
    const id = generatePrefixedId("gtpl");
    const now = new Date().toISOString();

    const row: Insertable<GatewayTemplatesDatabase["gateway_templates"]> = {
      id,
    ...
    await this.db.insertInto("gateway_templates").values(row).execute();

    const template = await this.findById(id);
    if (!template) {
      throw new Error(`Failed to create gateway template with id: ${id}`);
    }
   ...
   */
  async findById(id: string): Promise<GatewayTemplateEntity | null> {
    const row = await this.db
      .selectFrom("gateway_templates")
      .selectAll()
      .where("id", "=", id)
      .executeTakeFirst();
   ...
   */
  async list(organizationId: string): Promise<GatewayTemplateEntity[]> {
  ...
  async update(
    id: string,
    data: GatewayTemplateUpdateInput,
  ): Promise<GatewayTemplateEntity> {
    const now = new Date().toISOString();

    const updates: Updateable<GatewayTemplatesDatabase["gateway_templates"]> = {
      updated_at: now,
    };
   ...
   */
  private deserialize(row: RawTemplateRow): GatewayTemplateEntity {
