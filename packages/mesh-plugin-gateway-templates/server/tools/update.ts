/**
 * Gateway Templates Plugin - Update Tool
 *
 * Updates a gateway template. If changing required_apps, provide registry_id
...
import {
  GatewayTemplateUpdateInputSchema,
...
export const GATEWAY_TEMPLATE_UPDATE: ServerPluginToolDefinition = {
  name: "GATEWAY_TEMPLATE_UPDATE",
  description:
    "Update an existing gateway template. If updating required_apps, provide registry_id to auto-fetch details.",
  inputSchema: GatewayTemplateUpdateInputSchema,
  outputSchema: GatewayTemplateEntitySchema,

  handler: async (input, ctx) => {
    const typedInput = input as z.infer<
      typeof GatewayTemplateUpdateInputSchema
    >;
