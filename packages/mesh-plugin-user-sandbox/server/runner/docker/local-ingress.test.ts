...
describe("startLocalSandboxIngress", () => {
  it("routes non-_daemon paths to the dev port for the handle", async () => {
    const dev = await startUpstream("DEV");
    const daemon = await startUpstream("DAEMON");
    currentUpstreams.push(dev, daemon);

    const runner = runnerFor({
      alpha: { dev: dev.port, daemon: daemon.port },
    });
    ...
    expect(res.body).toBe("DEV");
    // The upstream received the original request bytes verbatim.
    expect(dev.received()).toContain("GET /index.html HTTP/1.1");
    expect(dev.received()).toContain("Host: alpha.sandboxes.localhost");
    expect(daemon.received()).toBe("");
  });

  it("routes /_daemon paths to the daemon port for the handle", async () => {
    const dev = await startUpstream("DEV");
    const daemon = await startUpstream("DAEMON");
    currentUpstreams.push(dev, daemon);
    ...
    expect(daemon.received()).toContain("GET /_daemon/ping HTTP/1.1");
    expect(dev.received()).toBe("");
  });

  it("routes the bare /_daemon path (no trailing segment) to the daemon port", async () => {
    const dev = await startUpstream("DEV");
    const daemon = await startUpstream("DAEMON");
    currentUpstreams.push(dev, daemon);

    const runner = runnerFor({
      beta: { dev: dev.port, daemon: daemon.port },
    });
    const { servers, port } = await startIngress(() => runner);
    currentServers = servers;

    const res = await sendHttp(port, "beta.sandboxes.localhost", "/_daemon");
    expect(res.status).toBe(200);
    expect(res.body).toBe("DAEMON");
    expect(dev.received()).toBe("");
  });
  ...
  it("accepts a host with an explicit port suffix (e.g. …:7070)", async () => {
    const dev = await startUpstream("DEV");
    currentUpstreams.push(dev);

    const runner = runnerFor({ alpha: { dev: dev.port } });
    const { servers, port } = await startIngress(() => runner);
    currentServers = servers;
  ...
  it("returns 404 when the dev port is missing even if daemon port exists", async () => {
    const daemon = await startUpstream("DAEMON");
    currentUpstreams.push(daemon);

    // Handle is known, but only the daemon port is registered. A non-_daemon
    // path must 404 (not fall back to daemon).
