...
interface K8sRecord {
   ...
   * repoAttached: true. When this differs from the running daemon's bootId,
   * rehydrate resets repoAttached so the next ensure re-bootstraps. Null
   * means the daemon predates the bootId field — fall back to stored flag.
   */
...
export class KubernetesSandboxRunner implements SandboxRunner {
  ...
  async delete(handle: string): Promise<void> {
    ...
    if (rec) {
      await stopDevServer(rec.daemonUrl, rec.token, LOG_LABEL);
  ...
  private async provision(
    ...
    const claim: SandboxClaim = {
      ...
      spec: {
        ...
        // Per-claim token. `valueFrom.secretKeyRef` isn't supported; RBAC on
        // the namespace is the secrecy boundary. Warm-pool off because the
        // operator rejects custom env on warm-pooled claims.
        env: [{ name: "DAEMON_TOKEN", value: token }],
        warmpool: "none",
        lifecycle: {
  ...
  private async rehydrate(
    ...
    if (!(await probeDaemonHealth(daemonUrl))) {
      this.closeForwarder(daemonForward);
      return null;
    }

    // Container restarted: pod is up but /app was wiped with its ephemeral
    // layer. Force re-bootstrap by resetting repoAttached. If the daemon
    // predates the bootId field, fall through to the persisted value.
    const currentBootId = await readDaemonBootId(daemonUrl);
    const rebooted =
      currentBootId !== null &&
      state.daemonBootId != null &&
      currentBootId !== state.daemonBootId;
    if (rebooted) {
      console.warn(
        `[${LOG_LABEL}] ${handle} pod rebooted (bootId ${state.daemonBootId} → ${currentBootId}); re-bootstrapping repo`,
      );
    }
