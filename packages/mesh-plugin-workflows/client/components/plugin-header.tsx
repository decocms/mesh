 ...
 *
 * Connection selector for the workflows plugin.
...
import { Dataflow03, ChevronDown, Check } from "@untitledui/icons";
  ...
  if (connections.length === 1) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>{selectedConnection?.title || "Workflows"}</span>
      </div>
    );
  }
  ...
  return (
    <div className="relative" ref={dropdownRef} onBlur={handleBlur}>
      ...
      >
        ...
        <span>{selectedConnection?.title || "Select provider"}</span>
        <ChevronDown size={14} />
      </button>
