...
Move per-tenant configuration (token, repo, runtime, env) out of `SandboxClaim.spec.env` and into a daemon-side bootstrap channel. The daemon becomes the configuration surface; `SandboxClaim.spec.env` shrinks to nothing tenant-relevant. Side effects: warm pools become viable (today blocked by per-claim env), and per-tenant config can be pushed post-bootstrap without re-rolling pods.
...
acquire bootstrapMutex
  validate schemaVersion, claimNonce, daemonToken (cheap field-level checks)
...
The state-store row (`PersistedK8sState`) holds `token`, `ensureOpts`, `claimNonce` (new), `bootstrappedAt` (new), `bootstrapHash` (new). All updates go through single-statement upserts:

- `provision` writes `{ token, ensureOpts, claimNonce }` before the bootstrap call.
...
Crash between the two writes is recoverable: `rehydrate` sees `ensureOpts` set + `bootstrappedAt` null → re-bootstrap using the persisted token and nonce.
...
- Apply to all `/_decopilot_vm/*` routes **except** the explicitly unauth'd set:
  ...
  - `POST /_decopilot_vm/bootstrap` — phase + nonce gated, not auth-gated; see Phase 1.
...
**Phase + nonce gated, not auth-gated.** The route runs through `bootstrapMutex` per the Concurrency section.

**Payload**:
```ts
{
  schemaVersion: 1;          // bumped on incompatible payload changes
  claimNonce: string;        // matches SandboxClaim annotation; see Trust framing
...
2. `claimNonce` matches `process.env.CLAIM_NONCE` (injected via downward API)? If not, `403`. Distinct from `409` on purpose: the caller is hitting the wrong daemon, not racing on the same one.
3. `daemonToken` ≥ 32 chars? If not, `400`.
4. Phase guard + idempotency check (Concurrency section).
...
### Trust framing for the bootstrap window

This is **not** a security model. The threat surface, bounded by NetworkPolicy + per-claim nonce:

1. **DoS** — anyone with network reach to `:9000` and the correct `claimNonce` can poison the daemon with a junk-but-well-formed payload. NetworkPolicy is the only mitigation; pod is wedged until recreate.
2. **Daemon takeover (multi-tenant)** — a peer with the path to `:9000` would also need to read the target claim's annotations to learn `claimNonce`. With cluster RBAC restricting `pods get/list` to the mesh ServiceAccount only, this raises the bar from "first to send wins" to "must compromise mesh's RBAC posture or steal its kubeconfig."
3. **Cross-tenant blind shot** — peers without nonce knowledge get `403`, not silent takeover. A `403`-counter metric is a regression alarm if the netpol relaxes.

**Per-claim nonce mechanics** (Phase 1, not future hardening):
- Mesh generates a 32-byte hex `claimNonce` per claim at provision time.
- Mesh writes it to `SandboxClaim.metadata.annotations["decocms.io/claim-nonce"]`.
- The pod template exposes the annotation as env `CLAIM_NONCE` via downward API.
- Daemon reads `process.env.CLAIM_NONCE` at startup; bootstrap rejects payloads whose `claimNonce` doesn't match.
- Annotations are metadata, so the operator's "no custom env on warm pods" check (`client.ts:68-74`) doesn't apply. **Verify against agent-sandbox v0.4.2 before merging** — see Phase 2's "Operator behavior to verify."

The defense for DoS is still NetworkPolicy: only mesh pods can reach `:9000`. Document this in the route handler so a future contributor relaxing the netpol doesn't think nonce alone covers them.

Future hardening (out of scope): projected SA token + TokenReview against the K8s API. With a SA token, bootstrap becomes "prove you're the specific mesh pod assigned to this tenant" instead of "prove you read the annotation."
...
- Unit: payload with mismatched `claimNonce` → `403` (regardless of phase).
...
- Generate `claimNonce` (32-byte hex) per claim, write to `metadata.annotations["decocms.io/claim-nonce"]`.
- Tenant labels stay on `additionalPodMetadata.labels` — those drive cAdvisor/kubelet metric attribution.
- The `RESERVED_ENV_KEYS` set + `buildEnvMap` logic moves verbatim into a new bootstrap-payload builder, which also stamps `schemaVersion` and `claimNonce`.
...
```
createSandboxClaim
  → persist {token, ensureOpts, claimNonce}        // tx 1
  → waitForSandboxReady
  → openForwarder
  → waitForDaemonHttp
...
A crash between tx 1 and tx 2 leaves the row with `ensureOpts` set + `bootstrappedAt` null. `rehydrate` recognizes this as "in-progress provision" and re-bootstraps using the persisted token + nonce.
...
| `/health.phase`     | Action                                                                 |
|---------------------|------------------------------------------------------------------------|
| `pending-bootstrap` | re-issue bootstrap from persisted `{token, ensureOpts, claimNonce}`    |
...
The re-bootstrap payload is composed from **three persisted fields** on the state-store row:
...
- `state.claimNonce` (new field) — required to pass the daemon's nonce check post-restart.

Treat as fatal if any of the three is missing on the row — falls through to recreate, same as today's null-opts case.
...
- Reads state-store row: has `ensureOpts` + `token` + `claimNonce`, missing `bootstrappedAt`.
...
Two assumptions need empirical confirmation against agent-sandbox v0.4.2:

1. **Warm-pool default**: `warmpool: undefined` in the claim resolves to the CRD's default of `"default"`. If the CRD default is `"none"`, dropping the explicit `"none"` changes nothing and warm pool stays cold-only.
2. **Annotations on warm-claimed pods**: the operator preserves custom `metadata.annotations` on warm-pool-claimed pods, even though it rejects custom `spec.env`. The `claimNonce` mechanism depends on this.

How to verify (1): spin up a `SandboxClaim` against v0.4.2 with no `spec.warmpool` field, no `spec.env`, and a populated `SandboxWarmPool`. Assert the pod was claimed from the warm pool (warm-pool replicas decrement, no fresh image pull).

How to verify (2): create a `SandboxClaim` with `metadata.annotations["decocms.io/claim-nonce"]: "test-value"` against a populated warm pool. Assert the claimed pod's annotation is preserved end-to-end and visible via downward API in the container.

If (2) fails, the nonce mechanism needs to fall back to a label-based downward API ref (labels are usually preserved by operators) or another path; do not ship Phase 1 without one of these.

### Tests

- Unit: `buildClaim` produces empty `spec.env`, populated `metadata.annotations["decocms.io/claim-nonce"]`.
- Unit: bootstrap payload round-trips through `buildEnvMap` shape, includes `schemaVersion` and `claimNonce`.
- Unit: re-bootstrap payload composes `state.token` + `state.ensureOpts` + `state.claimNonce` (regression guard for the triple-source shape).
...
- Zero `outcome=nonce_mismatch` events.
...
- Env vars retained: `DAEMON_BOOT_ID` (production attribution — mesh injects a UUID per-container so `/health.bootId` matches what mesh recorded; dev/test still auto-generates via `entry.ts:31-32`), `CLAIM_NONCE` (downward API).
...
The current `env:` block (lines 81-98) gets trimmed. Downward API for `CLAIM_NONCE` stays.
...
After cleanup: pod spec carries the `CLAIM_NONCE` downward-API ref + image + resources + security context + volumes only.
...
### Operator default for `warmpool` and annotations on warm pods

See Phase 2's "Operator behavior to verify." Both must be confirmed against v0.4.2 before Phase 1 ships, since the nonce mechanism depends on annotation preservation.
...
Run on every PR that touches `deploy/helm/sandbox-env/templates/networkpolicy.yaml` or the chart's pod selector labels. The test is also a forcing function for `claimNonce` annotation visibility — script asserts the nonce is unreachable from a non-mesh-RBAC pod's `kubectl get pods -o yaml`.

### Hardening path

Document in code that "claim-nonce + NetworkPolicy + mesh-RBAC" is the intentional trust model for Phase 1, and that future hardening is projected SA token + TokenReview, not a baked secret.
...
1. **Phase 0 + Phase 1 bundled** (single daemon image release).
   ...
   - Bootstrap mutex + atomic file persistence + `claimNonce`.
...
- No sidecars.
- `additionalPodMetadata.labels` keeps doing what it does today (tenant attribution for metrics). `additionalPodMetadata.annotations` gains one new key (`decocms.io/claim-nonce`); other annotations remain unused as a config channel.
...
3. **`bootstrap_outcome` metric attributes**: `outcome ∈ {success, conflict, validation_error, timeout, nonce_mismatch}`, plus `tenant_org_id`/`tenant_user_id` from `additionalPodMetadata.labels`. Cardinality risk on per-tenant attrs — bounded by active sandboxes, probably fine.
...
6. **`claimNonce` annotation visibility audit**: K8s `Pod` annotations are readable by anything with `pods get/list` RBAC in the namespace. Production cluster's mesh RBAC posture must restrict `pods get/list` to the mesh ServiceAccount only; otherwise the nonce is co-readable by other namespace-resident workloads. Track as a Phase 1 pre-merge audit item against the production cluster.
