...
| `agent_connection_attached` | F | `add-connection-dialog.tsx` — fires from all 5 attach paths once the connection_id is known: inline existing, clone success, connect_new success/oauth-fail, custom-create | `agent_id`, `connection_id`, `app_name` (nullable), `mode` (`existing` / `clone` / `new` / `custom`) — **the** authoritative "this connection is now on this agent" signal. Pairs with the org-scoped `connection_created` (which only fires for `clone` / `new` / `custom`, not `existing`). For `custom` mode, `app_name` is null at the dialog level — break down by app via the server `connection_created` event instead. |
...
| `ai_provider_oauth_failed` | F | `org-ai-providers.tsx:479` OAuth token-exchange `onError` (the exchange itself failed — bad code, server error, etc.) | `provider_id`, `error` — protocol/exchange failure only. NOT user-abandonment (see `ai_provider_oauth_timeout`). Race fixed: previously this could fire alongside a successful `ai_provider_oauth_succeeded` if the exchange took >2 min; now an `exchangeStarted` flag suppresses the timeout once the popup posts back. |
| `ai_provider_oauth_timeout` | F | `org-ai-providers.tsx:600` 2-min popup wait expired with no callback received | `provider_id` — user did not return from the OAuth popup within 2 minutes. Distinct from `ai_provider_oauth_failed` so funnel math (`connect_clicked` → `succeeded` vs abandonment) stays clean. |
