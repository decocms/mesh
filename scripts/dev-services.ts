...
async function ensurePostgres(): Promise<ServiceInfo> {
  ...
  const pg = new EmbeddedPostgres({
    ...
    persistent: true,
    onLog: (msg: string) => {
      // Suppress verbose postgres logs during startup
  ...
  // Create database (idempotent — embedded-postgres handles if exists)
  try {
  ...
  // Find the postgres PID from the postmaster.pid file
  ...
  if (existsSync(postmasterPidFile)) {
    const firstLine = readFileSync(postmasterPidFile, "utf8")
      .split("\n")[0]
      .trim();
    pgPid = Number.parseInt(firstLine, 10);
  }
...
async function stopPostgres(): Promise<void> {
  ...
  // Try embedded-postgres stop via pg_ctl
  const dataDir = join(SERVICES_DIR, "postgres", "data");
  try {
    const EmbeddedPostgres = (await import("embedded-postgres")).default;
    const pg = new EmbeddedPostgres({
...
async function downloadNats(): Promise<string> {
  ...
  // Extract using unzip (available on macOS/Linux) or PowerShell on Windows
...
async function ensureNats(): Promise<ServiceInfo> {
  ...
  // Step 3: Download (if needed) and start
...
function printTable(services: ServiceInfo[]) {
...
export { printTable, serviceStatus as getStatus };
